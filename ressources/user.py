...
class UserRegister(MethodView):
    ...
    def post(self, user_data):
        ...
        user = UserModel(username=user_data["username"], password=sha256_crypt.hash(user_data["password"]))
        db.session.add(user)
        db.session.commit()
...
class TokenRefresh(MethodView):
    ...
    def post(self):
        ...
        # if I want to block the refresh token after expiration
        # jti = get_jwt()["jti"]
        #BLOCKLIST.add(jti)
